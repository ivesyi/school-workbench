...
`standards_get` requires `packKey`, `version`, and at least one bounded filter (`dimensionKeys`, `practiceType`, or `criterionRefs`). A result is available only when the file Registry and persisted SQLite projection are both `active` and exactly match key/version/content hash/source fingerprint/criterion projection. The response contains only the selected Criteria, their required Constructs/ancestors, selected BehaviorAnchors, evidence guidance, counter indicators, relevant inference guardrails, source locators, and pack/version/hash provenance. The repository's real Schooling by Design and Data Wise packs remain `review`, so product queries currently return typed `no_active_pack`; tests use isolated active fixtures only.
