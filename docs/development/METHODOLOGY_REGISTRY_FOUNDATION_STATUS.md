...
The current Markdown proves a human-review baseline but does not prove that this specific machine translation has been explicitly approved as an active runtime standard. Activation requires a consultant review of the translated stable IDs, criterion wording, evidence guidance, boundaries/guardrails, source locators, source fingerprints, and canonical content hashes. A later change from `review` to `active` must be a new reviewed content commit; this slice does not auto-activate packs.
...
The source manifest fingerprints the consultant-local PDFs with SHA-256. The original PDFs are intentionally absent from the repository, fixtures, and application output.
...
- same key+version+hash and equal persisted projection: no-op;
