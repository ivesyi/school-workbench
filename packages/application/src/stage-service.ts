...
export class BaselineStageRecommendationEngine implements StageRecommendationEngine {
  async recommend(
    ...
    const isStudentFocused = /学生|学习|成绩|学业|学习体验/.test(context)
    const isPracticeFocused = /教师|课堂|教研|教案|教学|备课/.test(context)
    ...
    if (isStudentFocused) {
      return {
        ...
        focus: feedback
          ? `结合你的补充“${feedback.trim()}”，这个阶段现在最需要看到：成人实践的改变能够稳定对应到学生学习体验和结果的变化。`
          : '这个阶段现在最需要看到：成人实践的改变能够稳定对应到学生学习体验和结果的变化。',
        targets: {
          leadership: '领导团队持续追问改进行动是否真正带来学生学习变化。',
          critical_tasks: '关键改进任务明确连接学生学习证据，并据此调整下一步行动。',
          structure_systems: '形成稳定收集、讨论和使用学生学习证据的工作节奏。',
          culture: '团队能够基于证据讨论成效，而不是只汇报做了哪些事情。',
          capacity: '教师与中层能够从学生学习证据中判断实践效果并继续调整。',
        },
      }
    }

    if (isPracticeFocused) {
      return {
        title: '让改进进入教师实践',
        summary: '我理解这个学校目前大致处于“让改进进入教师实践”的阶段。',
        focus: feedback
          ? `结合你的补充“${feedback.trim()}”，这个阶段现在最需要看到：教师和教研团队开始用真实课堂证据持续调整实践，而不是只停留在理解和要求层面。`
          : '这个阶段现在最需要看到：教师和教研团队开始用真实课堂证据持续调整实践，而不是只停留在理解和要求层面。',
        targets: {
          leadership: '领导团队把关注点从布置要求转向支持教师实践改进和复盘。',
          critical_tasks: '课堂改进任务能够落到具体教学行为，并持续观察学生反应。',
          structure_systems: '教研、观察和复盘形成稳定节奏，能够支持教师持续试验和调整。',
          culture: '教师能够公开讨论真实课堂问题，并把失败和差异作为共同学习材料。',
          capacity: '教师能够基于课堂证据反思、调整教学，并与同伴共同改进。',
        },
      }
    }
    ...
    return {
      ...
      focus: feedback
        ? `结合你的补充“${feedback.trim()}”，这个阶段现在最需要看到：关键工作逐步不再只依赖校长，中层开始独立承担、推进和复盘，学校形成可重复的协作方式。`
        : '这个阶段现在最需要看到：关键工作逐步不再只依赖校长，中层开始独立承担、推进和复盘，学校形成可重复的协作方式。',
      targets: {
        leadership: '校长从直接代办转向明确方向、授权和复盘，中层承担真实责任。',
        critical_tasks: '至少一项关键改进任务由中层独立拆解、推进并根据结果调整。',
        structure_systems: '形成稳定的任务分工、推进节奏和复盘机制，不依赖临时推动。',
        culture: '中层能够公开讨论问题、提出不同判断并对推进结果负责。',
        capacity: '中层能够独立分析问题、制定行动、协同推进并完成复盘。',
      },
    }
  }
}
...
function toStageSummary(recommendation: StageRecommendation): StageSummaryView {
  return {
    ...
    targets: recommendation.targets.map((target) => ({
      ...
      label: targetLabels[target.dimensionKey],
      text: target.text,
    })),
  }
}
