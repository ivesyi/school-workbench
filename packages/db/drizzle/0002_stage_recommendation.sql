...
CREATE UNIQUE INDEX `stages_one_active_per_school` ON `stages` (`school_id`) WHERE `status` = 'active';
--> statement-breakpoint
CREATE UNIQUE INDEX `stages_one_planned_per_school` ON `stages` (`school_id`) WHERE `status` = 'planned';
--> statement-breakpoint
