...
- Methodology Registry 装配 + Pack 审核与激活机制（启动加载 + `syncRegistry` + 设置页高级设置下的审核工作台 + sign-off 落库 + 激活命令） — `docs/development/METHODOLOGY_PACK_ACTIVATION_STATUS.md`
...
下一轮方向：给两份 Pack 补齐翻译内容（10 条 criterion 的真实描述、维度归属、行为锚点边界、证据指引），补完后重新审核。待补清单见 `docs/development/METHODOLOGY_PACK_ACTIVATION_STATUS.md`。上一轮任务书（前瞻性文档，已执行）：`docs/development/METHODOLOGY_PACK_ACTIVATION_BRIEF.md`。
...
- **审核与激活机制已接入运行时，但两份 Pack 仍是 `status = review`，本轮没有激活。** 现有翻译的 10 条 criterion 的 `description` 与 `title` 完全相同、`dimensionKey` 全为 `null`、无行为锚点，不足以约束真实判断。因此 `standards_get` 在生产中仍返回 `no_active_pack`，`GroundedDiagnosisService` 仍不解锁，测试只使用隔离的 active fixture。
...
pnpm test
pnpm test:e2e
pnpm methodology:activate --pack <key> --version <version> [--apply]
```
